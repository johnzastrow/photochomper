...
def extract_xmp_metadata(filepath: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if xmp:
            ...
            try:
                # Keywords
                keywords_prop = xmp.get_property(
...
def discover_all_files(
    search_dir: str, excluded_extensions: List[str]
) -> Iterator[str]:
...
def create_sqlite_database(db_path: str) -> sqlite3.Connection:
    ...
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_datetime_run ON file_records(datetime_run)"
    )
    cursor.execute(
    ...
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_file_type ON file_records(file_type)"
    )
...
def insert_record_to_sqlite(conn: sqlite3.Connection, record: FileRecord):
    ...
    """,
        (
            ...
            record.dupe_sha256,
            record.master,
            record.char_length_name,
            record.char_length_path,
            record.file_size_bytes,
            record.file_created_datetime,
            record.file_modified_datetime,
            record.image_width_px,
            record.image_height_px,
            record.file_type,
            record.image_camera_make,
            record.image_camera_model,
            record.image_datetime_taken,
            record.image_quality_score,
            record.image_iptc_keywords,
            record.image_iptc_caption,
            record.image_xmp_keywords,
            record.image_xmp_title,
            record.image_latitude,
            record.image_longitude,
        ),
    )
...
def determine_master_files(records: List[FileRecord]) -> None:
    ...
    for sha256_hash, group_records in sha256_groups.items():
        if len(group_records) == 1:
            ...
            eligible_records = [
                r for r in group_records if "zip" not in r.file_name.lower()
            ]
            ...
            eligible_records.sort(
                key=lambda r: (r.char_length_name, r.char_length_path)
            )
...
def run_comprehensive_listing(
    search_dir: str, output_dir: str, excluded_extensions: List[str]
):
    ...
    try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
                for i in range(0, total_files, batch_size):
                    ...
                    future = executor.submit(
                        process_file_batch, batch_files, run_datetime
                    )
        ...
        duplicate_count = sum(
            1 for record in all_records if record.dupe_sha256 == "Yes"
        )
        master_count = sum(1 for record in all_records if record.master == "Yes")
        console.print(
            f"[cyan]Found {duplicate_count} files with duplicate SHA256 values[/cyan]"
        )
    ...
    # Log completion
    log_action(
        f"File listing completed: {processed_count} files processed | Search: {search_dir} | Output: {output_dir}"
    )
