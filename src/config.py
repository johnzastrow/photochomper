...
def select_list_config_file(config_dir: str = None) -> tuple:
    ...
    if not list_configs:
        console.print(
            f"[bold blue]No saved --list configurations found in {config_dir}.[/bold blue]"
        )
    ...
    console.print(
        f"[bold cyan]📋 Found {len(list_configs)} saved --list configuration(s):[/bold cyan]"
    )
    for idx, fname in enumerate(list_configs, 1):
        # Extract timestamp from filename for display
        try:
            timestamp_part = fname.replace("photochomper_list_config_", "").replace(
                ".listconf", ""
            )
    ...
    while True:
        ...
        if choice.isdigit():
            ...
            if 1 <= choice_num <= len(list_configs):
                ...
                console.print(
                    f"[green]Using saved configuration: {list_configs[choice_num - 1]}[/green]"
                )
...
# Update the default file types for --setup to match scanner.py's supported types.
DEFAULT_FILE_TYPES = [
    "jpg",
    "jpeg",
    "png",
