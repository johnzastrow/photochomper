...
def export_report(dupes: List[List[str]], formats: List[str] = ["csv", "json"], out_prefix: str = None, config_path: str = None, exec_time: float = None, progress_callback=None):
    ...
    if "csv" in formats:
        with open(f"{out_prefix}.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([
                "GroupID", "Algorithm", "Master", "File", "Name", "Path", "Size", "Created", "Modified",
                "Width", "Height", "FileType", "CameraMake", "CameraModel", "DateTaken",
                "QualityScore", "IPTCKeywords", "IPTCCaption", "XMPKeywords", "XMPTitle",
...
def create_sqlite_database(summary: List[Dict], db_path: str, hash_algorithm: str, exec_time: float = None):
    """
    ...
    if exec_time:
        cursor.execute('INSERT INTO metadata VALUES (?, ?)', ('execution_time_seconds', str(exec_time)))
    cursor.execute('INSERT INTO metadata VALUES (?, ?)', ('total_groups', str(len(summary))))
    cursor.execute('INSERT INTO metadata VALUES (?, ?)', ('total_files', str(sum(len(g["duplicates"]) + 1 for g in summary))))
