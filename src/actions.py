...
class ActionExecutor:
    ...
    def create_backup_dir(self, batch_id: str) -> Path:
        ...
        backup_dir = (
            ...
            / f"batch_{batch_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    def backup_file(self, source_path: str, backup_dir: Path) -> str:
        ...
        relative_path = (
            source.relative_to(source.anchor) if source.is_absolute() else source
        )
    ...
    def execute_action(self, action: FileAction, backup_dir: Path) -> bool:
        ...
        try:
            ...
            if action.action_type == ActionType.DELETE:
                ...
                log_action(
                    f"Tagged {action.source_path} with metadata: {action.metadata}"
                )
    ...
    def execute_batch(self, batch: ActionBatch, progress_callback=None) -> ActionBatch:
        ...
        log_action(
            f"Starting batch execution: {batch.batch_id} ({len(batch.actions)} actions)"
        )
        ...
        log_action(
            f"Batch execution completed: {batch.completed_actions} successful, {batch.failed_actions} failed"
        )
    ...
    def rollback_batch(self, batch: ActionBatch) -> bool:
        ...
        if not batch.backup_dir or not Path(batch.backup_dir).exists():
            log_action(
                f"Cannot rollback batch {batch.batch_id}: backup directory not found"
            )
            return False
            ...
            try:
                if action.action_type == ActionType.DELETE:
                    ...
                    if action.target_path and Path(action.target_path).exists():
                        ...
                        log_action(
                            f"Moved {action.target_path} back to {action.source_path}"
                        )
                        rollback_count += 1
                    ...
                    if action.target_path and Path(action.target_path).exists():
                        ...
                        log_action(
                            f"Renamed {action.target_path} back to {action.source_path}"
                        )
                        rollback_count += 1
    ...
    def save_batch_metadata(self, batch: ActionBatch, backup_dir: Path):
        ...
        for action_dict in batch_dict["actions"]:
            ...
            if "status" in action_dict:
                action_dict["status"] = (
                    action_dict["status"].value
                    if hasattr(action_dict["status"], "value")
                    else str(action_dict["status"])
                )
...
def create_delete_actions(
    duplicate_groups: List[List[str]], keep_strategy: str = "first"
) -> List[FileAction]:
    ...
    for group_id, group in enumerate(duplicate_groups):
        ...
        if keep_strategy == "first":
            ...
            files_with_size = [
                (f, os.path.getsize(f)) for f in group if os.path.exists(f)
            ]
...
def create_move_actions(
    duplicate_groups: List[List[str]], target_dir: str, keep_strategy: str = "first"
) -> List[FileAction]:
