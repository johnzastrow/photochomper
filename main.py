...
def run_summary_command(report_files, config_dir):
    ...
    if not report_files:
        console.print(
            "[bold blue]No report files specified. Searching for existing reports...[/bold blue]"
        )
        ...
        if not report_files:
            console.print(
                "[red]No report files found. Expected files like 'duplicates_report.csv' or 'duplicates_report.json'[/red]"
            )
            console.print(
                "[yellow]Run --search first to generate reports, or specify report files explicitly:[/yellow]"
            )
            ...
            console.print(
                "  Setup automatically ensures filenames contain 'report' for compatibility[/dim]"
            )
            return
    ...
    for file_path in report_files:
        ...
        if not file_path.lower().endswith((".csv", ".json")):
            console.print(
                f"[yellow]Skipping unsupported file type: {file_path}[/yellow]"
            )
            continue
    ...
    console.print(
        f"[bold blue]Generating markdown summary from {len(valid_files)} files...[/bold blue]"
    )
    ...
    try:
        ...
        console.print(
            f"[bold green]✅ Markdown summary generated: {output_file}[/bold green]"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description=f"PhotoChomper v{get_version()} - High-Performance Duplicate Photo Detection"
    )
    ...
    parser.add_argument(
        "--review",
        action="store_true",
        help="Interactive duplicate review with actions",
    )
    parser.add_argument(
        "--summary",
        nargs="*",
        help="Generate markdown summary from existing reports (CSV/JSON files)",
    )
    parser.add_argument("--schedule", type=int, help="Schedule search every N hours")
    parser.add_argument(
        "--list",
        action="store_true",
        help="List all files with comprehensive metadata and export to CSV/SQLite",
    )
    ...
    console.print(
        f"[bold blue]PhotoChomper v{get_version()}[/bold blue] - High-Performance Duplicate Photo Detection\n"
    )
    ...
    if args.review:
        interactive_duplicate_review(config)
    if args.schedule:
