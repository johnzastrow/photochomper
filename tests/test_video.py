...
def create_test_video(filepath: str, duration: int = 3, color: str = "red", resolution: str = "320x240"):
    ...
    try:
        ...
        cmd = [
            ...
            '-i', f'testsrc2=duration={duration}:size={resolution}:rate=30,format=yuv420p',
            '-f', 'lavfi',   # Audio input
        ...
        if result.returncode != 0:
            print(f"⚠️  Warning: Could not create test video with ffmpeg: {result.stderr}")
            return False
...
def test_video_metadata():
    ...
    try:
        ...
        if not test_videos:
            print("  ❌ No test videos could be created. Skipping video metadata tests.")
            return
        ...
        for video_path in test_videos:
            ...
            important_keys = [
                'file_type', 'size', 'duration', 'width', 'height', 
                'resolution_category', 'video_codec', 'audio_codec',
                'fps', 'video_bitrate', 'container_format'
            ]
    ...
    try:
        ...
        for algorithm in algorithms:
            ...
            for video_path in videos_created:
                ...
                error_info = f" (Error: {hash_result.error})" if hash_result.error else ""
                print(f"    {status} {Path(video_path).name}: {hash_result.hash_value[:16]}...{error_info}")
                ...
                for i in range(len(video_hashes)):
                    for j in range(i+1, len(video_hashes)):
                        ...
                        if not hash1.error and not hash2.error:
                            similarity = compute_video_similarity(hash1.file_path, hash2.file_path, algorithm)
