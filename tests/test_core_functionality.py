...
class TestMemoryOptimization(unittest.TestCase):
    ...
    def test_extreme_scenarios(self):
        ...
        for total_files, memory_mb, description in extreme_cases:
            ...
            print(f"{description:30s}: {total_files:7d} files, {memory_mb:5d}MB → {chunk_size:5d}/chunk, {estimated_chunks:4d} chunks")
            
            # Validation
            self.assertGreater(chunk_size, 0, f"Invalid chunk size for {description}")
            self.assertLessEqual(chunk_size, total_files, f"Chunk size exceeds files for {description}")
