...
class TestChunkSizeCalculation(unittest.TestCase):
    ...
    def test_automatic_chunk_sizing(self):
        ...
        for total_files, memory_mb, min_expected, max_expected in test_cases:
            ...
            self.assertLessEqual(chunk_size, total_files,
                "Chunk size should not exceed total files")
    ...
    def test_user_specified_chunk_size(self):
        ...
        self.assertEqual(result, total_files, "Should disable chunking when user specifies 0")
    ...
    def test_memory_tier_logic(self):
        ...
        self.assertLessEqual(very_low_memory_chunks, high_memory_chunks,
...
class TestChunkRecommendations(unittest.TestCase):
    ...
    def test_recommendation_structure(self):
        ...
        for rec_type in expected_types:
            ...
            required_fields = ['chunk_size', 'estimated_chunks', 'memory_usage_mb', 'description']
            for field in required_fields:
                self.assertIn(field, rec, f"Missing field {field} in {rec_type} recommendation")
    ...
    def test_recommendation_ordering(self):
        ...
        self.assertLess(conservative_mem, balanced_mem, 
            "Conservative should use less memory than balanced")
        self.assertLess(balanced_mem, performance_mem,
        ...
        self.assertLessEqual(conservative_chunk, performance_chunk,
    ...
    def test_edge_case_recommendations(self):
        ...
        for total_files, available_memory in edge_cases:
            ...
            for rec_type, rec in recommendations.items():
                ...
                expected_chunks = (total_files + rec['chunk_size'] - 1) // rec['chunk_size']
                self.assertEqual(rec['estimated_chunks'], expected_chunks,
...
class TestChunkedFileScanning(unittest.TestCase):
    ...
    def test_chunked_scanning_basic(self):
        ...
        chunk_size = 10
        chunks = list(scan_files_chunked(
            [self.temp_dir],
        ...
        self.assertEqual(len(chunks), expected_chunks, f"Should have {expected_chunks} chunks")
        
        # Each chunk (except possibly the last) should have chunk_size files
        for i, chunk in enumerate(chunks[:-1]):  # All but last chunk
            self.assertEqual(len(chunk), chunk_size, f"Chunk {i} should have {chunk_size} files")
        ...
        self.assertEqual(len(chunks[-1]), last_chunk_expected, "Last chunk should have correct size")
        ...
        self.assertEqual(len(all_files_from_chunks), len(self.test_files), 
    ...
    def test_chunked_scanning_file_types(self):
        ...
        for file_path in all_files:
            self.assertTrue(file_path.endswith('.jpg'), f"File {file_path} should be JPG")
        
        # Should have correct number of JPG files
        expected_jpg_count = len([f for f in self.test_files if f.endswith('.jpg')])
        self.assertEqual(len(all_files), expected_jpg_count, "Should have correct number of JPG files")
    ...
    def test_single_large_chunk(self):
        ...
        self.assertEqual(len(chunks[0]), len(self.test_files), "Single chunk should contain all files")
...
class TestMemoryOptimizationIntegration(unittest.TestCase):
    ...
    def test_memory_stats_integration(self, mock_memory):
        ...
        for mock_stats, description in memory_scenarios:
            ...
            self.assertLessEqual(chunk_size, total_files, f"Chunk size too large for {description}")
            
            print(f"\n{description}:")
            print(f"  Memory: {mock_stats.used_mb:.0f}MB used / {mock_stats.available_mb:.0f}MB available ({mock_stats.percent_used:.1f}%)")
...
class TestRealWorldScenarios(unittest.TestCase):
    ...
    def test_wedding_photography_scenario(self):
        ...
        self.assertLessEqual(chunk_size, 3000, "Chunk size should be manageable for 8GB system")
    ...
    def test_professional_studio_scenario(self):
        ...
        self.assertLessEqual(estimated_chunks, 300, "Should be efficient even for huge collections")
