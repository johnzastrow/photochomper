...
try:
    from src.scanner import (
        HashCache, HashResult, HashAlgorithm, FileType,
        compute_perceptual_hash, compute_video_hash,
        get_optimal_chunk_size,
        find_similarity_groups_efficient, find_similarity_groups_lsh
    )
    ...
    def test_concurrent_cache_operations(self):
        ...
        def worker(worker_id):
            try:
                ...
                retrieved = cache.get_cached_hash(f"/test/worker_{worker_id}.jpg", HashAlgorithm.DHASH)
...
class RegressionTestV310ProgressTracking(unittest.TestCase):
    ...
    def test_memory_conscious_chunking_still_works(self):
        ...
        test_cases = [
            (1000, None, 100, 2000),    # total_files, user_chunk, min_expected, max_expected
            (50000, None, 500, 2000),
...
class RegressionTestV200EnhancedTUI(unittest.TestCase):
    ...
    def test_config_system_still_works(self):
        ...
        self.assertEqual(loaded_config["directories_to_scan"], ["/test/dir1", "/test/dir2"])
...
class RegressionTestVersionTracking(unittest.TestCase):
    ...
    def test_version_info_accessible(self):
        ...
        found_current = any(entry.startswith(current_entry) for entry in version_history)
        self.assertTrue(found_current, f"Current version {version} not found in history")
...
def run_performance_regression_test():
    ...
    if 'cache_creation_time' in previous_baseline:
        creation_regression = current_cache_creation_time / previous_baseline['cache_creation_time']
        ...
        if creation_regression > 1.5:
            print(f"⚠️  WARNING: Cache creation is {creation_regression:.1f}x slower than v3.1.4")
