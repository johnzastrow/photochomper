...
from src.scanner import find_duplicates, HashAlgorithm, ProgressStats, get_chunk_size_recommendations, get_optimal_chunk_size
    ...
    def enhanced_progress_callback(stats: ProgressStats):
        ...
        if stats.phase != last_phase or len(progress_log) % 5 == 0:
            ...
            if stats.total_files > 0:
                ...
                eta_info = f" | ETA: {format_time(stats.estimated_completion_time)}" if stats.estimated_completion_time > 0 else ""
                print(f"{emoji} {stats.phase}")
                print(f"   Progress: {progress_info} | Elapsed: {elapsed_str} ({phase_elapsed_str} this phase){eta_info}")
            else:
...
def main():
    ...
    try:
        ...
        progress_updates, search_time = demonstrate_progress_tracking(temp_dir, optimal_chunk)
