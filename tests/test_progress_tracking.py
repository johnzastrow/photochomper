...
class TestProgressCallbacks(unittest.TestCase):
    ...
    def test_progress_callback_called(self):
        ...
        # Verify progression
        file_counts = [call['files_processed'] for call in callback_calls if call['total_files'] > 0]
        if len(file_counts) > 1:
            # Should show increasing file counts
            self.assertLessEqual(file_counts[0], file_counts[-1], "File counts should increase over time")
    ...
    def test_progress_callback_skip_sha256(self):
        ...
        self.assertEqual(len(sha256_phases), 0, "Should not have SHA256 phase when skipped")
        
        # Should have similarity phase
        similarity_phases = [phase for phase in phases if 'Similarity' in phase or 'DHASH' in phase]
        self.assertGreater(len(similarity_phases), 0, "Should have similarity detection phase")
    ...
    def test_full_progress_tracking_flow(self):
        ...
        def comprehensive_callback(stats: ProgressStats):
            progress_log.append({
                ...
                'eta': f"{stats.estimated_completion_time:.2f}s" if stats.estimated_completion_time > 0 else "N/A"
            })
        ...
        # Verify results
        self.assertGreaterEqual(len(groups), 1, "Should find at least one duplicate group")
        ...
        if len(progress_log) > 1:
            ...
            self.assertEqual(first_phase, "File Discovery", "Should start with file discovery")
...
class TestMemoryAndChunking(unittest.TestCase):
    ...
    def test_chunk_size_recommendations(self):
        ...
        for total_files, available_memory in test_cases:
            ...
            for name, rec in recommendations.items():
                print(f"  {name.title()}: {rec['chunk_size']} files/chunk, "
