...
from src.actions_clean import ActionExecutor, create_delete_actions
...
def test_action_creation():
    ...
    for strategy in strategies:
        ...
        print(
            f"  {strategy.capitalize()} strategy: {len(delete_actions)} delete actions (expected: {total_deletes})"
        )

        # Show examples
        for action in delete_actions[:2]:
            print(
                f"    - Delete: {Path(action.source_path).name} (group {action.metadata['group_id']})"
            )
